...
use gul::platform::package_support::{PackageManager, PackageCategory};

#[test]
fn test_package_manager_creation() {
    let pm = PackageManager::new();
    assert!(pm.count() > 0, "Package manager should have default packages");
}
...
fn test_list_by_language() {
    ...
    assert!(rust_packages.len() >= 7, "Should have multiple Rust packages");
    
    let python_packages = pm.list_packages_by_language("python");
    assert!(python_packages.len() >= 6, "Should have multiple Python packages");
    
    let js_packages = pm.list_packages_by_language("javascript");
    assert!(js_packages.len() >= 6, "Should have multiple JavaScript packages");
}
...
fn test_package_has_dependencies() {
    ...
    assert!(!axum.dependencies.is_empty(), "Axum should have dependencies");
...
fn test_multi_language_support() {
    ...
    let languages: Vec<String> = pm.list_packages()
        .iter()
