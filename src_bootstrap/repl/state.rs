...
impl ReplState {
    ...
    pub fn eval_line(&mut self, line: &str) -> bool {
        ...
        if self.in_multiline {
            if trimmed.is_empty() {
                ...
                self.multiline_buf.push(line.to_string());
                self.output
                    .push(OutputEntry::Continuation(line.to_string()));
                return false;
            }
        }
    ...
    fn evaluate_source(&mut self, source: &str) {
        ...
        let wrapped = if source.contains("mn:")
            || source.contains("@fn ")
        ...
        match parser.parse() {
            ...
            Err(e) => {
                self.output
                    .push(OutputEntry::Error(format!("Parse error: {}", e)));
            }
        }
    }
    ...
    fn handle_command(&mut self, cmd: &str) -> bool {
        ...
        match parts[0] {
            ":help" => {
                self.output.push(OutputEntry::Result(
                    "Commands: :help :clear :load <file> :reset :quit".to_string(),
                ));
                self.output.push(OutputEntry::Result(
                    "Keys: Up/Down=history, Ctrl-C=cancel, Ctrl-L=clear, Ctrl-D=quit".to_string(),
                ));
            }
            ":clear" => {
                self.output.clear();
            ...
            ":reset" => {
                ...
                self.output
                    .push(OutputEntry::Result("State reset.".to_string()));
            }
            ":load" => {
                if parts.len() > 1 {
                    ...
                    match std::fs::read_to_string(path) {
                        Ok(source) => {
                            ...
                            self.output
                                .push(OutputEntry::Result(format!("Loaded: {}", path)));
                        }
                        Err(e) => {
                            self.output
                                .push(OutputEntry::Error(format!("Failed to load: {}", e)));
                        }
                    }
                } else {
                    self.output
                        .push(OutputEntry::Error("Usage: :load <file.mn>".to_string()));
                }
            }
            ":quit" => return true,
            _ => {
                self.output
                    .push(OutputEntry::Error(format!("Unknown command: {}", parts[0])));
            }
        }
        false
    }
