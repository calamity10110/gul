...
fn test_parse_simple_definition() {
    ...
    match &program.statements[0] {
        crate::frontend::ast::Statement::Definition { name, .. } => assert_eq!(name, "x"),
...
fn test_parse_function() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::Function { name, params, .. } => {
            assert_eq!(name, "add");
...
fn test_parse_ui_sprite_expression() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::Definition { name, value } => {
            assert_eq!(name, "slider");
            // Check if the value is a UiSprite expression
            match value {
                crate::frontend::ast::Expression::UiSprite { .. } => {} // Success
...
fn test_parse_custom_block() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::ForeignBlock { language, .. } => {
...
fn test_parse_if_elif_else() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::If {
            then_body,
            else_body,
            ..
        } => {
...
fn test_parse_for_loop() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::For { variable, .. } => {
...
fn test_parse_while_loop() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::While { .. } => {
            // Success
        }
...
fn test_parse_main_function() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::Main { .. } => {
            // Success
        }
...
fn test_parse_import_statement() {
    ...
    assert_eq!(program.statements.len(), 1);
    match &program.statements[0] {
        crate::frontend::ast::Statement::Import(modules) => {
...
fn test_v21_function_call_with_brackets() {
    ...
    match &program.statements[0] {
        crate::frontend::ast::Statement::Definition { name, value } => {
            ...
            match value {
                crate::frontend::ast::Expression::Call { function: _, args } => {
...
fn test_v21_list_with_parens() {
    ...
    match &program.statements[0] {
        crate::frontend::ast::Statement::Definition { name, value } => {
            assert_eq!(name, "data");
            match value {
                crate::frontend::ast::Expression::List(elements) => {
...
fn test_v21_dict_with_brackets() {
    ...
    match &program.statements[0] {
        crate::frontend::ast::Statement::Definition { name, value } => {
            ...
            match value {
                crate::frontend::ast::Expression::Dict(pairs) => {
...
fn test_v21_empty_collections() {
    ...
    match &program.statements[0] {
        crate::frontend::ast::Statement::Definition { value, .. } => match value {
            crate::frontend::ast::Expression::List(elements) => assert_eq!(elements.len(), 0),
...
fn test_v21_mixed_bracket_nesting() {
    ...
    match &program.statements[0] {
        crate::frontend::ast::Statement::Definition { name, value } => {
            assert_eq!(name, "x");
            match value {
                crate::frontend::ast::Expression::Call { args, .. } => {
