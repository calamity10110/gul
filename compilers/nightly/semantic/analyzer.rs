...
impl Scope {
    ...
    pub fn borrow_mut(&mut self, name: String) -> Result<(), String> {
        ...
        if let Some(symbol) = self.symbols.get_mut(&name) {
            if symbol.ownership_mode == "move" {
                return Err(format!(
                    "Cannot mutably borrow '{}': value has been moved",
                    name
                ));
            }
            ...
            if !symbol.is_mutable {
                return Err(format!(
                    "Cannot mutably borrow '{}': not declared as 'var'",
                    name
                ));
            }
            symbol.is_borrowed = true;
...
pub fn create_analyzer() -> SemanticAnalyzer {
    ...
    // Define builtins
    analyzer.current_scope.symbols.insert(
        "print".to_string(),
        Symbol {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_autograd_begin".to_string(),
        Symbol {
            name: "gul_autograd_begin".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_autograd_end".to_string(),
        Symbol {
            name: "gul_autograd_end".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_make_var".to_string(),
        Symbol {
            name: "gul_make_var".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_var_val".to_string(),
        Symbol {
            name: "gul_var_val".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_var_grad".to_string(),
        Symbol {
            name: "gul_var_grad".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_var_add".to_string(),
        Symbol {
            name: "gul_var_add".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
                params: vec!["any".to_string(), "any".to_string()],
                return_type: "any".to_string(),
            }),
            ownership_mode: "owned".to_string(),
            is_borrowed: false,
    ...
    analyzer.current_scope.symbols.insert(
        "gul_var_mul".to_string(),
        Symbol {
            name: "gul_var_mul".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_var_sin".to_string(),
        Symbol {
            name: "gul_var_sin".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
    ...
    analyzer.current_scope.symbols.insert(
        "gul_backward".to_string(),
        Symbol {
            name: "gul_backward".to_string(),
            symbol_type: "fn".to_string(),
            is_mutable: false,
            is_function: true,
            signature: Some(MethodSignature {
...
impl SemanticAnalyzer {
    ...
    pub fn analyze_let_stmt(&mut self, stmt: &LetStmt) {
        ...
        if self.current_scope.exists_in_current(stmt.name.clone()) {
            self.error(
                format!("Variable '{}' already defined", stmt.name),
                stmt.node.line,
                stmt.node.column,
            );
        }
        ...
        self.current_scope.define(
            ...
            Symbol {
                ...
                symbol_type: if !stmt.type_annotation.is_empty() {
                    stmt.type_annotation.clone()
                } else {
                    value_type
                },
                is_mutable: false,
    ...
    pub fn analyze_function_decl(&mut self, stmt: &FunctionDecl) {
        ...
        if self.current_scope.exists_in_current(stmt.name.clone()) {
            self.error(
                format!("Function '{}' already defined", stmt.name),
                stmt.node.line,
                stmt.node.column,
            );
        ...
        for param in &stmt.parameters {
            param_types.push(if param.type_annotation.is_empty() {
                "any".to_string()
            } else {
                param.type_annotation.clone()
            });
        }
        let return_type = if stmt.return_type.is_empty() {
            "void".to_string()
        } else {
        ...
        for param in &stmt.parameters {
            self.current_scope.define(
                ...
                Symbol {
                    ...
                    symbol_type: if param.type_annotation.is_empty() {
                        "any".to_string()
                    } else {
                        param.type_annotation.clone()
                    },
    ...
    pub fn analyze_if_stmt(&mut self, stmt: &IfStmt) {
        ...
        if !cond_type.is_empty()
            && cond_type != "bool".to_string()
            && cond_type != "any".to_string()
        {
    ...
    pub fn analyze_import(&mut self, stmt: &ImportStmt) {
        ...
        let std_libs = vec![
            "std", "math", "io", "tensor", "simd", "flow", "alloc", "data", "ml",
        ]; // simplified list
        ...
        if root == "std" {
            if stmt.module_path.len() > 1 {
                ...
                if !std_libs.contains(&sub.as_str())
                    && sub != "box"
                    && sub != "rc"
                    && sub != "arc"
                    && sub != "refcell"
    ...
    pub fn analyze_expression(&mut self, expr: &Expression) -> String {
        ...
        match expr {
            Expression::Literal(l) => {
                if l.value_type == TokenType::Integer {
                    return "int".to_string();
                } else if l.value_type == TokenType::Float {
                    return "float".to_string();
                } else if l.value_type == TokenType::String {
                    return "str".to_string();
                } else if l.value_type == TokenType::TrueKeyword
                    || l.value_type == TokenType::FalseKeyword
                {
                    ...
                    if ident.name == "print" {
                        ...
                        for (i, arg) in expr.arguments.iter().enumerate() {
                            ...
                            if !self.types_compatible(expected_type.clone(), arg_type.clone()) {
                                ...
                                self.error(
                                    format!(
                                        "Argument {} of '{}' mismatch: expected {}, got {}",
                                        i + 1,
                                        ident.name,
                                        expected_type,
                                        arg_type
    ...
    pub fn analyze_index(&mut self, expr: &IndexExpr) -> String {
        ...
        if object_type.starts_with("list<") {
            ...
            if let Some(comma_pos) = object_type.find(',') {
                return object_type[comma_pos + 1..]
                    .trim_end_matches(">")
                    .trim()
                    .to_string();
            }
    ...
    pub fn types_compatible(&self, expected: String, actual: String) -> bool {
        ...
        if expected == "str".to_string()
            && (actual == "int".to_string() || actual == "float".to_string())
        {
    ...
    pub fn analyze_struct_decl(&mut self, stmt: &StructDecl) {
        ...
        if self.current_scope.exists_in_current(stmt.name.clone()) {
            self.error(
                format!("Struct '{}' already defined", stmt.name),
                stmt.node.line,
                stmt.node.column,
            );
        }
    ...
    pub fn analyze_foreign_block(&mut self, stmt: &ForeignCodeBlock) {
        ...
        if !valid_langs.contains(&stmt.language.as_str()) {
            self.warn(
                format!("Unknown foreign language '{}'", stmt.language),
                stmt.node.line,
                stmt.node.column,
            );
        }
