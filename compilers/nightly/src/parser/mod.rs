...
impl Parser {
    ...
    fn parse_pattern(&mut self) -> Result<Pattern> {
        match self.peek().token_type {
            ...
            TokenType::Float => {
                let val = self.advance().lexeme.parse().unwrap();
                Ok(Pattern::Literal(ExpressionKind::Float(val)))
            }
            TokenType::Boolean => {
    ...
    fn parse_type(&mut self) -> Result<Type> {
        // Simple type parsing for primitive types
        // Extended types could be added later
        if self.peek().token_type == TokenType::Identifier {
            ...
            // Potentially handle @list, @dict if tokens exist, for now just primitives
            Err(anyhow!("Expected type identifier"))
        }
    }
    ...
    fn parse_function(&mut self) -> Result<Statement> {
        ...
        while !self.check(&TokenType::RightParen) && !self.is_at_end() {
            ...
            let mut mode = OwnershipMode::Unknown;

            if self.check(&TokenType::Colon) {
                self.advance(); // consume ':'
                
                if self.check(&TokenType::Borrow) {
                     self.advance();
                     mode = OwnershipMode::Borrow;
                } else if self.check(&TokenType::Ref) {
                     self.advance();
                     mode = OwnershipMode::Ref;
                } else if self.check(&TokenType::Move) {
                     self.advance();
                     mode = OwnershipMode::Move;
                } else if self.check(&TokenType::Kept) {
                     self.advance();
                     mode = OwnershipMode::Kept;
                }
                
                // Parse type (required if colon is present)
                if self.check(&TokenType::Identifier) || self.check(&TokenType::AtList) || self.check(&TokenType::AtSet) || self.check(&TokenType::AtDict) {
                    ty = self.parse_type()?;
                }
            }
            
            params.push(Param { name: param_name, ty, mode });
        ...
        match &self.peek().token_type {
            ...
            TokenType::LeftParen => {
                ...
                if self.check(&TokenType::Comma) {
                     ...
                     self.expect(TokenType::RightParen)?;
                     // Tuples as Lists for MVP
                     Ok(Expression { kind: ExpressionKind::List(items), ty: Type::List(Box::new(Type::Unknown)) })
                } else {
                     self.expect(TokenType::RightParen)?;
                     Ok(expr)
                }
            ...
            TokenType::AtList => {
                ...
                // Next must be [ (list literal) or ( (constructor, if we supported it)
                // Spec says @list[1, 2], so we expect LeftBracket
            ...
            TokenType::AtSet => {
                ...
                Ok(Expression { kind: ExpressionKind::Set(items), ty: Type::Set(Box::new(Type::Unknown)) })
            }
            TokenType::AtDict => {
                ...
                self.expect(TokenType::RightBrace)?;
                Ok(Expression { kind: ExpressionKind::Dict(items), ty: Type::Dict(Box::new(Type::Unknown), Box::new(Type::Unknown)) })
            }
            TokenType::Decorator | TokenType::AtInt | TokenType::AtFloat | TokenType::AtStr | TokenType::AtBool => {
                let name = self.advance().lexeme.clone();
                Ok(Expression { kind: ExpressionKind::Identifier(name), ty: Type::Unknown })
            }
    ...
    // Skips newlines, indents, and dedents (for inside brackets)
