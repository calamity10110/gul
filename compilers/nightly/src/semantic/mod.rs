    ...
    fn analyze_statement(&mut self, stmt: Statement) -> Result<Statement> {
        match stmt {
            Statement::Let { name, ty, value } => {
                ...
                // Float Inference Logic
                if ty.is_none() {
                     if let Type::Integer = typed_value.ty {
                         // Promote Integer to Float by default (Spec v3.2)
                         // EXCEPTION: Do not promote Function Calls (including @int cast).
                         // This respects explicit return types and casts.
                         if !matches!(typed_value.kind, ExpressionKind::Call { .. }) {
                             if let ExpressionKind::Integer(val) = typed_value.kind {
                                 typed_value = Expression { kind: ExpressionKind::Float(val as f64), ty: Type::Float };
                             } else {
                                typed_value = Expression {
                                    kind: ExpressionKind::Call {
                                        callee: Box::new(Expression { kind: ExpressionKind::Identifier("@flt".to_string()), ty: Type::Unknown }), 
                ...
                } else if let Some(ref target_ty) = ty {
                    // Type Checking
                    if typed_value.ty != *target_ty && typed_value.ty != Type::Unknown {
                        // Allow some coercions? Int -> Float is valid?
                        // Spec says "Explicit Int with type annotation". So Int is required.
                        // If typed_value is Float (from mixed arithmetic), and target is Int... Type Mismatch?
                        // "type mismatch error" (4.5)
                        // So we strictly check match.
                        // Exception: Literals? No, literals are pure until promoted.
                        // If type annotation is Int, literal 0 stays Int.
                        // So simple check is fine.
                        if matches!(target_ty, Type::Float) && matches!(typed_value.ty, Type::Integer) {
                             // Allow auto-promote Int to Float if target is Float
                             if let ExpressionKind::Integer(val) = typed_value.kind {
                                 typed_value = Expression { kind: ExpressionKind::Float(val as f64), ty: Type::Float };
                             } else {
                                  typed_value = Expression {
                            ...
                            // Strict mismatch
                            // return Err(...) or warning.
                            // For now let's error if strict.
                            // But for MVP verification, I'll log/warn or return error.
                        }
                    }
                }

                let final_ty = typed_value.ty.clone();
                self.define_symbol(name.clone(), final_ty, false);
                Ok(Statement::Let { name, ty, value: typed_value })
            }
            Statement::Var { name, ty, value } => {
                ...
                let final_ty = typed_value.ty.clone();
                self.define_symbol(name.clone(), final_ty, true);
                Ok(Statement::Var { name, ty, value: typed_value })
            }
            ...
            Statement::Function { name, params, body } => {
                ...
                // Define params
                for param in &params {
                    self.define_symbol(param.name.clone(), param.ty.clone(), true);
                }
            ...
            Statement::Match { value, arms } => {
                ...
                for arm in arms {
                    ...
                    // If pattern binds variable, define it.
                    let new_pattern = match &arm.pattern {
                        Pattern::Identifier(name) => {
                            self.define_symbol(name.clone(), typed_value.ty.clone(), true);
                            arm.pattern.clone()
                        },
                        _ => arm.pattern.clone()
                    };
                ...
                Ok(Statement::Match { value: typed_value, arms: new_arms })
            }
            _ => Ok(stmt)
        }
    }
    ...
    fn analyze_expression(&mut self, expr: Expression) -> Result<Expression> {
        match expr.kind {
            ...
            ExpressionKind::BinaryOp { left, op, right } => {
                ...
                let ty = match (&typed_left.ty, &typed_right.ty) {
                    (Type::Integer, Type::Integer) => match op {
                     ...
                     (Type::Float, Type::Float) | (Type::Integer, Type::Float) | (Type::Float, Type::Integer) => match op {
                         BinaryOperator::Equal | BinaryOperator::NotEqual | 
                        BinaryOperator::Less | BinaryOperator::Greater |
                        BinaryOperator::LessEq | BinaryOperator::GreaterEq | BinaryOperator::In => Type::Boolean,
                        _ => Type::Float,
                    },
                    (Type::String, _) | (_, Type::String) => {
                         if let BinaryOperator::Add = op {
                             Type::String 
                         } else if let BinaryOperator::In = op {
                             Type::Boolean
                         } else {
                             Type::Unknown
                         }
            ...
            ExpressionKind::Set(items) => {
                ...
                Ok(Expression { 
                    kind: ExpressionKind::Set(typed_items), 
                    ty: Type::Set(Box::new(item_type)) 
                })
            }
            ExpressionKind::Dict(items) => {
                ...
                for (i, (key, val)) in items.into_iter().enumerate() {
                    let typed_key = self.analyze_expression(key)?;
                    let typed_val = self.analyze_expression(val)?;
                    
                    if i == 0 {
                        key_type = typed_key.ty.clone();
                        val_type = typed_val.ty.clone();
                    }
                    typed_items.push((typed_key, typed_val));
                }
                
                Ok(Expression {
                    kind: ExpressionKind::Dict(typed_items),
