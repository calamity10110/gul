...
impl CraneliftBackend {
    ...
    fn generate_string_literal(text: &str, ctx: &mut GenContext) -> Result<Value> {
        ...
        let data_id = if let Some(id) = ctx.string_literals.get(text) {
            *id
            ...
            ctx.module
                .define_data(id, ctx.data_description)
    ...
    pub fn generate(&mut self, program: &Program) -> Result<Vec<u8>> {
        ...
        let gul_print_float_id = module
            .declare_function("gul_print_float", Linkage::Import, &sig_pf)
        ...
        let gul_input_str_id = module
            .declare_function("gul_input_str", Linkage::Import, &sig_input_str)
        ...
        let gul_table_alloc_id = module
            .declare_function("gul_table_alloc", Linkage::Import, &sig_table_alloc)
        ...
        let gul_table_set_col_name_id = module
            .declare_function("gul_table_set_col_name", Linkage::Import, &sig_tbl_col)
        ...
        let gul_table_set_row_id = module
            .declare_function("gul_table_set_row", Linkage::Import, &sig_tbl_row)
        ...
        let gul_malloc_id = module
            .declare_function("gul_malloc", Linkage::Import, &sig_malloc)
        ...
        let gul_input_int_id = module
            .declare_function("gul_input_int", Linkage::Import, &sig_input_int)
        ...
        let gul_input_flt_id = module
            .declare_function("gul_input_flt", Linkage::Import, &sig_input_flt)
        ...
        let gul_string_concat_id = module
            .declare_function("gul_string_concat", Linkage::Import, &sig_concat)
        ...
        let gul_list_alloc_id = module
            .declare_function("gul_list_alloc", Linkage::Import, &sig_list_alloc)
        ...
        sig_list_push.params.push(AbiParam::new(types::I64));
        let gul_list_push_id = module
            .declare_function("gul_list_push", Linkage::Import, &sig_list_push)
        ...
        let gul_list_get_id = module
            .declare_function("gul_list_get", Linkage::Import, &sig_list_get)
        ...
        let gul_list_len_id = module
            .declare_function("gul_list_len", Linkage::Import, &sig_list_len)
        ...
        let gul_dict_alloc_id = module
            .declare_function("gul_dict_alloc", Linkage::Import, &sig_dict_alloc)
        ...
        sig_dict_set.params.push(AbiParam::new(types::I64));
        sig_dict_set.params.push(AbiParam::new(types::I64));
        let gul_dict_set_id = module
            .declare_function("gul_dict_set", Linkage::Import, &sig_dict_set)
        ...
        let gul_dict_get_id = module
            .declare_function("gul_dict_get", Linkage::Import, &sig_dict_get)
        ...
        let gul_int_to_string_id = module
            .declare_function("gul_int_to_string", Linkage::Import, &sig_int_to_str)
        ...
        let gul_float_to_string_id = module
            .declare_function("gul_float_to_string", Linkage::Import, &sig_flt_to_str)
        ...
        let gul_set_alloc_id = module
            .declare_function("gul_set_alloc", Linkage::Import, &sig_set_alloc)
        ...
        sig_set_add.params.push(AbiParam::new(types::I64));
        let gul_set_add_id = module
            .declare_function("gul_set_add", Linkage::Import, &sig_set_add)
            .map_err(|e| anyhow!("Failed to declare gul_set_add: {}", e))?;

        // Declare format strings
        self.data_description
            .define(b"%ld\n\0".to_vec().into_boxed_slice());
        let format_int_id = module
            .declare_data("fmt_int", Linkage::Local, true, false)
            .map_err(|e| anyhow!("Failed to declare data: {}", e))?;
        module
            .define_data(format_int_id, &self.data_description)
            .map_err(|e| anyhow!("Failed to define data: {}", e))?;
        self.data_description.clear();

        self.data_description
            .define(b"%s\n\0".to_vec().into_boxed_slice());
        let format_str_id = module
            .declare_data("fmt_str", Linkage::Local, true, false)
            .map_err(|e| anyhow!("Failed to declare data: {}", e))?;
        module
            .define_data(format_str_id, &self.data_description)
            .map_err(|e| anyhow!("Failed to define data: {}", e))?;
        self.data_description.clear();
        ...
        for name in builtin_names {
            ...
            if name.contains("math") || name.contains("ml") {
                ...
                if name.ends_with("pow")
                    || name.ends_with("atan2")
                    || name.ends_with("min")
                    || name.ends_with("max")
                {
                ...
                if name.ends_with("alloc") {
                    ...
                    sig.params.push(AbiParam::new(types::I64));
                    sig.params.push(AbiParam::new(types::I64));
                } else if name.ends_with("sum") || name.ends_with("mean") {
                    // ptr, size -> f64
                    sig.params.push(AbiParam::new(types::I64));
                    sig.params.push(AbiParam::new(types::I64));
                    sig.returns.push(AbiParam::new(types::F64));
        ...
        for func in &program.functions {
            ...
            let internal_name = if func.name == "main" {
                "gul_user_main".to_string()
            } else {
                func.name.clone()
            };
            let func_id = module
                .declare_function(&internal_name, Linkage::Local, &sig)
    ...
    fn generate_main(
...

        {
            ...
            for stmt in &program.main_entry {
                if Self::generate_statement(stmt, &mut gen_ctx)? {
                    break;
                }
            }
            ...
            if program.main_entry.is_empty() {
                if let Some(fid) = functions.get("main") {
                    let func_ref = gen_ctx
                        .module
                        .declare_func_in_func(*fid, gen_ctx.builder.func);
            ...
            let is_terminated =
                if let Some(last_inst) = gen_ctx.builder.func.layout.last_inst(current_block) {
                    gen_ctx.builder.func.dfg.insts[last_inst]
                        .opcode()
                        .is_terminator()
                } else {
                    false
    ...
    fn generate_function(
        ...
        let func_id = *functions
            .get(&func_def.name)
            .ok_or(anyhow!("Function ID not found"))?;
...

        {
            ...
            for (i, param) in func_def.parameters.iter().enumerate() {
                ...
                *gen_ctx.var_counter += 1;
                gen_ctx
                    .builder
                    .declare_var(var, gen_ctx.builder.func.dfg.value_type(val));
                gen_ctx.builder.def_var(var, val);

                // Determine param type
                let type_name = if !param.type_annotation.is_empty() {
                ...
                gen_ctx
                    .variables
                    .insert(param.name.clone(), (var, type_name));
            }
        ...
        match stmt {
            Statement::LetDecl(let_stmt) => {
                ...
                if !let_stmt.type_annotation.is_empty() {
                    ...
                    if expected == "str" {
                        if actual == "int" {
                            let func_ref = ctx
                                .module
                                .declare_func_in_func(ctx.gul_int_to_string_id, ctx.builder.func);
                            let call_res = ctx.builder.ins().call(func_ref, &[val]);
                            val = ctx.builder.inst_results(call_res)[0];
                        } else if actual == "float" {
                            let func_ref = ctx
                                .module
                                .declare_func_in_func(ctx.gul_float_to_string_id, ctx.builder.func);
                    ...
                    if expected == "bool" && actual == "str" {
                        ...
                        sig.params.push(AbiParam::new(types::I64));
                        sig.returns.push(AbiParam::new(types::I64));
                        let fid = ctx.module.declare_function(
                            "gul_str_to_bool",
                            Linkage::Import,
                            &sig,
                        )?;
                ...
                *ctx.var_counter += 1;
                ...
                ctx.variables
                    .insert(let_stmt.name.clone(), (var, type_name));
                Ok(false)
            }
            Statement::VarDecl(var_stmt) => {
                ...
                *ctx.var_counter += 1;
                ...
                ctx.variables
                    .insert(var_stmt.name.clone(), (var, type_name));
                Ok(false)
            }
            ...
            Statement::ForStmt(for_stmt) => {
                    ...
                    if binop.operator == TokenType::DotDot {
                        ...
                        *ctx.var_counter += 1;
                        ...
                        let cond =
                            ctx.builder
                                .ins()
                                .icmp(IntCC::SignedLessThan, current_index, end_val);
                        ...
                        ctx.variables
                            .insert(for_stmt.variable.clone(), (index_var, "int".to_string()));
                ...
                let len_func_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_list_len_id, ctx.builder.func);
                ...
                *ctx.var_counter += 1;
                ...
                let cond = ctx
                    .builder
                    .ins()
                    .icmp(IntCC::SignedLessThan, current_index, len_val);
                ctx.builder.ins().brif(cond, body_block, &[], exit, &[]);

                ctx.builder.switch_to_block(body_block);

                // 5. Load current element: var = gul_list_get(iterable, index)
                let get_func_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_list_get_id, ctx.builder.func);
                let get_res = ctx
                    .builder
                    .ins()
                    .call(get_func_ref, &[iterable_val, current_index]);
                ...
                *ctx.var_counter += 1;
                ...
                let old_var = ctx.variables.get(&for_stmt.variable).cloned();
                ctx.variables
                    .insert(for_stmt.variable.clone(), (loop_var, "int".to_string()));
            ...
            Statement::IfStmt(if_stmt) => {
                ...
                ctx.builder
                    .ins()
                    .brif(cond_val, then_block, &[], current_false_block, &[]);
                ...
                for elif in &if_stmt.elif_clauses {
                    ...
                    ctx.builder
                        .ins()
                        .brif(elif_cond, next_true_block, &[], next_false_block, &[]);
            ...
            Statement::ImportStmt(_) => Ok(false),
            _ => Ok(false),
        }
    }
        ...
        match expr {
            ...
            Expression::BinaryOp(binop) => {
                ...
                if binop.operator == TokenType::Plus && !is_float {
                    ...
                    if left_t == "str" || right_t == "str" {
                        ...
                        let func_ref = ctx
                            .module
                            .declare_func_in_func(ctx.gul_string_concat_id, ctx.builder.func);
                ...
                match binop.operator {
                    TokenType::Plus => {
                        if is_float {
                            Ok(ctx.builder.ins().fadd(left, right))
                        } else {
                    ...
                    TokenType::Minus => {
                        if is_float {
                            Ok(ctx.builder.ins().fsub(left, right))
                        } else {
                            Ok(ctx.builder.ins().isub(left, right))
                        }
                    }
                    TokenType::Star => {
                        if is_float {
                            Ok(ctx.builder.ins().fmul(left, right))
                        } else {
                            Ok(ctx.builder.ins().imul(left, right))
                        }
                    }
                    TokenType::Slash => {
                        if is_float {
                            Ok(ctx.builder.ins().fdiv(left, right))
                        } else {
                            Ok(ctx.builder.ins().sdiv(left, right))
                        }
                    }
                    TokenType::Greater => {
                        if is_float {
                            Ok(ctx.builder.ins().fcmp(FloatCC::GreaterThan, left, right))
                        } else {
                            Ok(ctx
                                .builder
                                .ins()
                    ...
                    TokenType::Less => {
                        if is_float {
                            Ok(ctx.builder.ins().fcmp(FloatCC::LessThan, left, right))
                        } else {
                            Ok(ctx.builder.ins().icmp(IntCC::SignedLessThan, left, right))
                        }
                    }
                    TokenType::GreaterEq => {
                        if is_float {
                    ...
                    TokenType::EqualEqual => {
                        if is_float {
                            Ok(ctx.builder.ins().fcmp(FloatCC::Equal, left, right))
                        } else {
                            Ok(ctx.builder.ins().icmp(IntCC::Equal, left, right))
                        }
                    }
                    TokenType::NotEqual => {
                        if is_float {
                            Ok(ctx.builder.ins().fcmp(FloatCC::NotEqual, left, right))
                        } else {
                            Ok(ctx.builder.ins().icmp(IntCC::NotEqual, left, right))
                        }
                    }
                    TokenType::And => Ok(ctx.builder.ins().band(left, right)),
                    TokenType::Or => Ok(ctx.builder.ins().bor(left, right)),
                    _ => Ok(ctx.builder.ins().iconst(types::I64, 0)),
                }
            }
            Expression::Attribute(attr) => {
                ...
                if found {
                    ...
                    Ok(ctx
                        .builder
                        .ins()
                        .load(types::I64, flags, obj_val, offset as i32))
            ...
            Expression::Index(idx_expr) => {
                ...
                if obj_type == "dict" {
                    let func_ref = ctx
                        .module
                        .declare_func_in_func(ctx.gul_dict_get_id, ctx.builder.func);
                ...
                if let Expression::Attribute(attr) = call.callee.as_ref() {
                    if attr.attribute == "push" || attr.attribute == "append" {
                        ...
                        if !call.arguments.is_empty() {
                            ...
                            let func_ref = ctx
                                .module
                                .declare_func_in_func(ctx.gul_list_push_id, ctx.builder.func);
                ...
                if let Expression::Identifier(ident) = call.callee.as_ref() {
                    if let Some(layout) = ctx.struct_layouts.get(&ident.name) {
                        ...
                        if let Some(malloc_id) = ctx.builtins.get("gul_malloc") {
                            let malloc_ref = ctx
                                .module
                                .declare_func_in_func(*malloc_id, ctx.builder.func);
                        ...
                        && !call.arguments.is_empty()
                    {
                        ...
                        if arg_type == types::F64 {
                            let func_ref = ctx
                                .module
                                .declare_func_in_func(ctx.gul_print_float_id, ctx.builder.func);
                        ...
                        if is_str {
                            let printf_ref = ctx
                                .module
                                .declare_func_in_func(ctx.printf_id, ctx.builder.func);
                    ...
                    if ident.name == "len" && !call.arguments.is_empty() {
                        ...
                        let fid = if arg_type == "dict" {
                            ...
                            sig.returns.push(AbiParam::new(types::I64));
                            ctx.module
                                .declare_function("gul_dict_len", Linkage::Import, &sig)
                                .map_err(|e| anyhow!("{}", e))?
                            ...
                            sig.returns.push(AbiParam::new(types::I64));
                            ctx.module
                                .declare_function("gul_string_len", Linkage::Import, &sig)
                                .map_err(|e| anyhow!("{}", e))?
                        } else {
                            ctx.gul_list_len_id
                        };
                    ...
                    if ident.name == "input" {
                        ...
                        let func_ref = ctx
                            .module
                            .declare_func_in_func(ctx.gul_input_str_id, ctx.builder.func);
                    ...
                    if ident.name.starts_with('@') && !call.arguments.is_empty() {
                        ...
                        match ident.name.as_str() {
                            "@flt" | "@float" => {
                                ...
                                if inner_type == types::F64 {
                                    ...
                                    return Ok(ctx
                                        .builder
                                        .ins()
                                        .fcvt_from_sint(types::F64, inner_val));
                                }
                            }
                            "@int" => {
                                // Cast to i64
                                if inner_type == types::I64 {
                                    ...
                                    return Ok(ctx
                                        .builder
                                        .ins()
                                        .fcvt_to_sint(types::I64, inner_val));
                                } else {
                            ...
                            "@str" | "@string" => {
                                ...
                                if arg_type == types::F64 {
                                    let func_ref = ctx.module.declare_func_in_func(
                                        ctx.gul_float_to_string_id,
                                        ctx.builder.func,
                                    );
                                    let call_result =
                                        ctx.builder.ins().call(func_ref, &[inner_val]);
                                    return Ok(ctx.builder.inst_results(call_result)[0]);
                                } else {
                                    let func_ref = ctx.module.declare_func_in_func(
                                        ctx.gul_int_to_string_id,
                                        ctx.builder.func,
                                    );
                                    let call_result =
                                        ctx.builder.ins().call(func_ref, &[inner_val]);
            ...
            Expression::Table(table) => {
                ...
                let alloc_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_table_alloc_id, ctx.builder.func);
                let call_result = ctx
                    .builder
                    .ins()
                    .call(alloc_ref, &[col_count_val, row_count_val]);
                let table_ptr = ctx.builder.inst_results(call_result)[0];

                // Set column names
                let set_col_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_table_set_col_name_id, ctx.builder.func);
                for (i, col_name) in table.columns.iter().enumerate() {
                    ...
                    ctx.builder
                        .ins()
                        .call(set_col_ref, &[table_ptr, idx_val, name_val]);
                }

                // Set rows
                let set_row_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_table_set_row_id, ctx.builder.func);
                let malloc_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_malloc_id, ctx.builder.func);
                ...
                for (i, row) in table.rows.iter().enumerate() {
                    ...
                    let row_size = ctx
                        .builder
                        .ins()
                        .iconst(types::I64, (row.values.len() * 8) as i64);
                    ...
                    ctx.builder
                        .ins()
                        .call(set_row_ref, &[table_ptr, idx_val, name_val, values_ptr]);
                }
            ...
            Expression::List(list) => {
                ...
                let alloc_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_list_alloc_id, ctx.builder.func);
            ...
            Expression::Dict(dict) => {
                ...
                let alloc_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_dict_alloc_id, ctx.builder.func);
                let call_result = ctx.builder.ins().call(alloc_ref, &[capacity_val]);
                ...
                let set_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_dict_set_id, ctx.builder.func);
                for (key_expr, value_expr) in &dict.pairs {
                    ...
                    ctx.builder
                        .ins()
                        .call(set_ref, &[dict_ptr, key_val, value_val]);
                }
            ...
            Expression::Set(set) => {
                ...
                let alloc_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_set_alloc_id, ctx.builder.func);
                let call_result = ctx.builder.ins().call(alloc_ref, &[capacity_val]);
                ...
                let add_ref = ctx
                    .module
                    .declare_func_in_func(ctx.gul_set_add_id, ctx.builder.func);
            ...
            Expression::DataFrame(df_expr) => {
                ...
                let func_id =
                    ctx.module
                        .declare_function("gul_frame_create", Linkage::Import, &sig)?;
                let func_ref = ctx.module.declare_func_in_func(func_id, ctx.builder.func);
                ...
                let set_col_id = ctx.module.declare_function(
                    "gul_frame_set_column_name",
                    Linkage::Import,
                    &set_col_sig,
                )?;
                let set_col_ref = ctx
                    .module
                    .declare_func_in_func(set_col_id, ctx.builder.func);
                ...
                for (i, col_name) in df_expr.columns.iter().enumerate() {
                    ...
                    ctx.builder
                        .ins()
                        .call(set_col_ref, &[df_ptr, idx_val, name_val]);
                }
