...
pub fn tokenize(source: String) -> Vec<Token> {
    ...
    while l_pos < l_len {
        ...
        if ch_char == ' ' || ch_char == '\t' || ch_char == '\r' {
            ...
            let next_char = if l_pos + 1 < l_len {
                Some(chars[l_pos + 1])
            } else {
                None
            };
            ...
            if ch_char == '|' && next_char == Some('>') {
                ...
                match ch_char {
                    ...
                    '(' => {
                        t_p = TokenType::LeftParen;
                        bracket_level += 1;
                    }
                    ')' => {
                        t_p = TokenType::RightParen;
                        if bracket_level > 0 {
                            bracket_level -= 1;
                        }
                    }
                    '[' => {
                        t_p = TokenType::LeftBracket;
                        bracket_level += 1;
                    }
                    ']' => {
                        t_p = TokenType::RightBracket;
                        if bracket_level > 0 {
                            bracket_level -= 1;
                        }
                    }
                    '{' => {
                        t_p = TokenType::LeftBrace;
                        bracket_level += 1;
                    }
                    '}' => {
                        t_p = TokenType::RightBrace;
                        if bracket_level > 0 {
                            bracket_level -= 1;
                        }
                    }
