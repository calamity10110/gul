...
impl CodeGenerator {
    ...
    pub fn generate_let(&mut self, stmt: &LetStmt) {
        ...
        if !stmt.type_annotation.is_empty() { code = format!("{}: {}", code, self.map_type(stmt.type_annotation.clone())) }
    ...
    pub fn generate_function(&mut self, stmt: &FunctionDecl) {
        ...
        for param in &stmt.parameters {
            ...
            if !param.type_annotation.is_empty() { param_str = format!("{}: {}", param_str, self.map_type(param.type_annotation.clone())) }
            params.push(param_str);
        }
        signature = signature + &params.join(", ") + ")";
        if !stmt.return_type.is_empty() { signature = format!("{} -> {}", signature, self.map_type(stmt.return_type.clone())) }
        signature = signature + " {";
        self.emit(signature);
        self.indent();
    ...
    pub fn generate_expression_str(&mut self, expr: &Expression)  ->  String {
        match expr {
            Expression::Literal(lit) =>
                if lit.value_type == TokenType::String { return self.generate_string_literal(lit.clone()) }
                else { return self.generate_literal(lit.clone()) }
    ...
    pub fn generate_binary_op(&mut self, expr: &BinaryOpExpr)  ->  String {
        ...
        let op = if op_t == TokenType::Plus { "+".to_string() }
        else if op_t == TokenType::Minus { "-".to_string() }
        else if op_t == TokenType::Star { "*".to_string() }
        else if op_t == TokenType::Slash { "/".to_string() }
        else if op_t == TokenType::Percent { "%".to_string() }
        else if op_t == TokenType::DoubleStar { return format!("{}.pow({})", left, right) }
        else if op_t == TokenType::EqualEqual { "==".to_string() }
        else if op_t == TokenType::NotEqual { "!=".to_string() }
        else if op_t == TokenType::Greater { ">".to_string() }
        else if op_t == TokenType::Less { "<".to_string() }
        else if op_t == TokenType::GreaterEq { ">=".to_string() }
        else if op_t == TokenType::LessEq { "<=".to_string() }
        else if op_t == TokenType::And { "&&".to_string() }
        else if op_t == TokenType::Or { "||".to_string() }
