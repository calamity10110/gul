...
impl CodeGenerator {
    ...
    pub fn generate_program(&mut self, program: Program) {
        ...
        self.emit_raw("// Generated by GUL compiler".to_string());
    ...
    pub fn generate_let(&mut self, stmt: &LetStmt) {
        ...
        if !stmt.type_annotation.is_empty() {
            code = format!("{}: {}", code, self.map_type(stmt.type_annotation.clone()))
        }
        ...
        for param in &stmt.parameters {
            ...
            if !param.type_annotation.is_empty() {
                param_str = format!(
                    "{}: {}",
                    param_str,
                    self.map_type(param.type_annotation.clone())
                )
            }
            params.push(param_str);
        }
        signature = signature + &params.join(", ") + ")";
        if !stmt.return_type.is_empty() {
            signature = format!(
                "{} -> {}",
                signature,
                self.map_type(stmt.return_type.clone())
            )
        }
        signature = signature + " {";
        self.emit(signature);
    ...
    pub fn generate_return(&mut self, stmt: &ReturnStmt) {
        ...
        if let Some(val) = &stmt.value {
            let value = self.generate_expression_str(val);
    ...
    pub fn generate_import(&mut self, stmt: &ImportStmt) {
        ...
        let module = stmt.module_path.join("::");

        if (stmt.items).len() > 0 {
            let items = stmt.items.join(", ");
            self.emit_raw("use ".to_string() + &module + "::{" + &items + "};");
        } else {
    ...
    pub fn generate_expression_str(&mut self, expr: &Expression) -> String {
        match expr {
            Expression::Literal(lit) => {
                if lit.value_type == TokenType::String {
                    return self.generate_string_literal(lit.clone());
                } else {
                    return self.generate_literal(lit.clone());
                }
            }
            ...
            Expression::Grouped(g) => {
                return "(".to_string()
                    + &self.generate_expression_str(&g.expression)
                    + ".to_string().to_string())"
            }
    ...
    pub fn generate_literal(&mut self, expr: LiteralExpr) -> String {
        ...
        if expr.value_type == TokenType::Integer || expr.value_type == TokenType::Float {
            return expr.value;
        } else if expr.value_type == TokenType::TrueKeyword {
            return "true".to_string();
        } else if expr.value_type == TokenType::FalseKeyword {
            return "false".to_string();
        } else if expr.value_type == TokenType::Boolean {
            return expr.value;
        } else if expr.value_type == TokenType::NoneLiteral {
    ...
    pub fn generate_binary_op(&mut self, expr: &BinaryOpExpr) -> String {
        ...
        let op_t = expr.operator.clone();

        let op = if op_t == TokenType::Plus {
            "+".to_string()
        } else if op_t == TokenType::Minus {
            "-".to_string()
        } else if op_t == TokenType::Star {
            "*".to_string()
        } else if op_t == TokenType::Slash {
            "/".to_string()
        } else if op_t == TokenType::Percent {
            "%".to_string()
        } else if op_t == TokenType::DoubleStar {
            return format!("{}.pow({})", left, right);
        } else if op_t == TokenType::EqualEqual {
            "==".to_string()
        } else if op_t == TokenType::NotEqual {
            "!=".to_string()
        } else if op_t == TokenType::Greater {
            ">".to_string()
        } else if op_t == TokenType::Less {
            "<".to_string()
        } else if op_t == TokenType::GreaterEq {
            ">=".to_string()
        } else if op_t == TokenType::LessEq {
            "<=".to_string()
        } else if op_t == TokenType::And {
            "&&".to_string()
        } else if op_t == TokenType::Or {
            "||".to_string()
        } else if op_t == TokenType::DotDot {
            return format!("{}..{}", left, right);
        } else if op_t == TokenType::DotDotEq {
            return format!("{}..={}", left, right);
        } else {
            "?".to_string()
        };
    ...
    pub fn map_type(&self, gul_type: String) -> String {
        if gul_type == "int".to_string() {
            return "i64".to_string();
        }
        if gul_type == "float".to_string() {
            return "f64".to_string();
        }
        if gul_type == "str".to_string() {
            return "String".to_string();
        }
        if gul_type == "bool".to_string() {
            return "bool".to_string();
        }
        if gul_type == "list".to_string() {
            return "Vec<_>".to_string();
        }
        if gul_type == "dict".to_string() {
            return "HashMap<_, _>".to_string();
        }
        if gul_type == "set".to_string() {
            return "HashSet<_>".to_string();
        }
        if gul_type == "tuple".to_string() {
            return "(_)".to_string();
        }
        return gul_type;
