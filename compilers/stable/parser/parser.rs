...
pub fn get_precedence(token_type: TokenType) -> Precedence {
    ...
    } else if token_type == TokenType::Less
    ...
    } else if token_type == TokenType::Star
    ...
    } else if token_type == TokenType::LeftParen
...
impl Parser {
    ...
    pub fn parse_prefix(&mut self) -> Expression {
        ...
        if t_type == TokenType::Identifier {
            ...
            return Expression::Literal(LiteralExpr {
                node: ASTNode { line: 1, column: 1 },
                value: token.value,
                value_type: TokenType::String,
            });
        } else if t_type == TokenType::TrueKeyword {
            return Expression::Literal(LiteralExpr {
                node: ASTNode { line: 1, column: 1 },
                value: "true".to_string(),
                value_type: TokenType::TrueKeyword,
            });
        } else if t_type == TokenType::FalseKeyword {
            return Expression::Literal(LiteralExpr {
                node: ASTNode { line: 1, column: 1 },
                value: "false".to_string(),
                value_type: TokenType::FalseKeyword,
            });
        } else if t_type == TokenType::LeftBracket {
            // List: [1, 2, 3]
            let mut elements = vec![];
            while self.current().token_type != TokenType::RightBracket
                && self.current().token_type != TokenType::Eof
            {
                self.skip_newlines();
                if self.current().token_type == TokenType::RightBracket {
                    break;
                }
            ...
            if self.current().token_type == TokenType::RightBrace {
                ...
                return Expression::Dict(DictExpr {
                    node: ASTNode { line: 1, column: 1 },
                    pairs: vec![],
                });
            }
        ...
        } else if t_type == TokenType::Newline
        ...
        // Handle type constructor tokens as identifiers for backend processing
            ...
            if self.current().token_type == TokenType::LeftBrace {
                ...
                self.expect(
                    ...
                    "Expected '}' after dict entries".to_string(),
                );
                return Expression::Dict(DictExpr {
                    node: ASTNode { line: 1, column: 1 },
                    pairs: pairs,
            ...
            if self.current().token_type == TokenType::LeftBrace {
                ...
                self.expect(
                    TokenType::RightBrace,
                    "Expected '}' after set elements".to_string(),
                );
                return Expression::Set(SetExpr {
                    node: ASTNode { line: 1, column: 1 },
                    elements: elements,
        ...
        return Expression::Literal(LiteralExpr {
            ...
            value_type: TokenType::NoneLiteral,
        });
    }
    pub fn parse_infix(&mut self, left: Expression, precedence: Precedence) -> Expression {
            ...
            || token.token_type == TokenType::DotDot
        {
            ...
            match right {
                Expression::Call(mut call_expr) => {
                    ...
                    let placeholder_pos = call_expr.arguments.iter().position(
                        |arg| matches!(arg, Expression::Identifier(ident) if ident.name == "_"),
                    );
        ...
        } else if token.token_type == TokenType::EqualEqual
            || token.token_type == TokenType::NotEqual
            || token.token_type == TokenType::Less
            || token.token_type == TokenType::LessEq
            || token.token_type == TokenType::Greater
            || token.token_type == TokenType::GreaterEq
        {
            let right = self.parse_expression(precedence);
            return Expression::BinaryOp(BinaryOpExpr {
                node: ASTNode { line: 1, column: 1 },
        ...
        while self.current_pos < (self.tokens).len() {
            ...
            if tok.token_type == TokenType::Eof {
                ...
                if self.current_pos < (self.tokens).len()
                    && self.tokens[self.current_pos].token_type == TokenType::Colon
                {
                    self.advance(); // consume ':'
                }
                ...
                while self.current_pos < (self.tokens).len()
                    && self.tokens[self.current_pos].token_type == TokenType::Newline
                {
                    self.advance();
                }
                ...
                // Expect indent
                if self.current_pos < (self.tokens).len()
                    && self.tokens[self.current_pos].token_type == TokenType::Indent
                {
                    self.advance();
                }
                ...
                while self.current_pos < (self.tokens).len() {
                    ...
                    if t.token_type == TokenType::Dedent {
                        self.advance();
                        break;
                    } else if t.token_type == TokenType::Newline
...
impl Parser {
    ...
    pub fn parse_let_statement(&mut self) -> Statement {
        ...
        if self.match_token(TokenType::Colon) {
            let type_token =
                self.expect(TokenType::Identifier, "Expected type after ':'".to_string());
            type_annotation = type_token.value;
        }
        self.expect(
            TokenType::Equal,
            "Expected '=' in let declaration".to_string(),
        );
        let value = self.parse_expression(Precedence::NoPrec);
    ...
    pub fn parse_var_statement(&mut self) -> Statement {
        ...
        self.expect(
            TokenType::Equal,
            "Expected '=' in var declaration".to_string(),
        );
        let value = self.parse_expression(Precedence::NoPrec);
    ...
    pub fn parse_decorated_declaration(&mut self) -> Statement {
        ...
        while self.current().token_type == TokenType::Identifier
            && self.current().value.starts_with("@")
        {
            let token = self.advance();
        ...
        if self.match_token(TokenType::Fn) || self.match_token(TokenType::AtFn) {
            ...
            || self.current().token_type == TokenType::AtFn
        {
            return self.parse_function_declaration(decorators);
        } else {
            ...
            return Statement::PassStmt(PassStmt {
    ...
    pub fn parse_function_declaration(&mut self, decorators: Vec<String>) -> Statement {
        ...
        if self.current().token_type == TokenType::AtInt
            || self.current().token_type == TokenType::AtStr
        {
        ...
        while !self.match_token(TokenType::RightParen) {
            ...
            if !self.match_token(TokenType::Comma) {
                self.expect(
                    TokenType::RightParen,
                    "Expected ')' || ',' in parameters".to_string(),
                );
                break;
        ...
        self.expect(
            TokenType::Indent,
            "Expected indented block for function body".to_string(),
        );
    ...
    pub fn parse_parameter(&mut self) -> Parameter {
        ...
        } else if self.current().token_type == TokenType::LeftBrace
            || self.current().token_type == TokenType::LeftBracket
        {
            ...
            self.expect(
                TokenType::Identifier,
                "Expected parameter name or destructuring pattern".to_string(),
            );
        }
        ...
        if self.match_token(TokenType::Colon) {
            ...
            if self.current().token_type == TokenType::Borrow
                || self.current().token_type == TokenType::Ref
                || self.current().token_type == TokenType::Move
                || self.current().token_type == TokenType::Kept
            {
                ownership_mode = self.advance().value;
            }
            let type_token =
                self.expect(TokenType::Identifier, "Expected parameter type".to_string());
            type_annotation = type_token.value;
        }
    ...
    pub fn parse_tabl_literal(&mut self) -> Expression {
        ...
        if self.current().token_type == TokenType::Identifier
            && self.current().value == "sparse".to_string()
        {
            is_sparse = true;
            self.advance();
        }
    ...
    pub fn parse_try_statement(&mut self) -> Statement {
        ...
        self.expect(
            TokenType::Indent,
            "Expected indented block for try".to_string(),
        );
        ...
        while self.match_token(TokenType::Catch) {
            ...
            self.expect(
                TokenType::Indent,
                "Expected indented block for catch".to_string(),
            );
        ...
        if self.match_token(TokenType::Finally) {
            ...
            self.expect(
                TokenType::Indent,
                "Expected indented block for finally".to_string(),
            );
    ...
    pub fn parse_for_statement(&mut self) -> Statement {
        ...
        let var_token = self.expect(
            TokenType::Identifier,
            "Expected variable in for loop".to_string(),
        );
    ...
    pub fn parse_expression_or_assignment(&mut self) -> Statement {
        ...
        if self.current().token_type == TokenType::Equal
            || self.current().token_type == TokenType::PlusEq
            || self.current().token_type == TokenType::MinusEq
            || self.current().token_type == TokenType::StarEq
            || self.current().token_type == TokenType::SlashEq
        {
            let op = self.advance();
            let value = self.parse_expression(Precedence::NoPrec);
    ...
    pub fn parse_struct_declaration(&mut self) -> Statement {
        ...
        self.expect(
            TokenType::Indent,
            "Expected indented block for struct body".to_string(),
        );
        ...
        while !self.is_at_end() && self.current().token_type != TokenType::Dedent {
            ...
            if self.current().token_type == TokenType::Fn {
                ...
                let field_type =
                    self.expect(TokenType::Identifier, "Expected field type".to_string());
    ...
    pub fn parse_foreign_block(&mut self, lang: &str) -> Statement {
        ...
        self.expect(
            TokenType::Colon,
            "Expected ':' after foreign block tag".to_string(),
        );
        self.skip_newlines();
    ...
    pub fn parse_parallel_for_statement(&mut self) -> Statement {
        ...
        if self.current().token_type == TokenType::For {
            ...
            self.expect(
                TokenType::For,
                "Expected 'for' after 'parallel'".to_string(),
            );
            return Statement::PassStmt(PassStmt {
                node: create_node(p_token.line, p_token.column),
            });
        }
    }
    ...
    pub fn parse_flow_declaration(&mut self) -> Statement {
        ...
        if self.current().token_type == TokenType::Var {
            ...
            self.expect(
                TokenType::Var,
                "Expected 'var' declaration for flow".to_string(),
            );
            return Statement::PassStmt(PassStmt {
                node: create_node(token.line, token.column),
            });
