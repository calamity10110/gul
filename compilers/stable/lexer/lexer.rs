...
pub fn tokenize(source: String)  ->  Vec<Token> {
    ...
    while l_pos < l_len {
        ...
        if ch == " ".to_string() || ch == "\t".to_string() || ch == "\r".to_string() {
            ...
            if ch == "|".to_string() && next_char == Some('>') {
            ...
            else if ch == "+".to_string() { t_p = TokenType::Plus }
                ...
                // Fallback for single pipe (Bitwise Or or just a pipe) - assuming Or for now if defined or Error
                // Ideally should add BitOr to TokenType, but for now treating as error if not pipeline
                 t_p = TokenType::Error; 
            }
